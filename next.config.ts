...
const nextConfig: NextConfig = {
  ...
  async headers() {
    return [
      ...
      {
        // Self-hosted Arabic faces (app/globals.css); a weight never
        // changes in place, so the files can be cached for a year.
        source: '/fonts/:path*',
  ...
  experimental: {
    serverActions: {
      // Vercel caps every Node function request body at 4.5MB on all
      // plans (platform 413 before the function runs), so anything larger
      // here is fiction — resolved 2026-09 (engineering audit DEPS-06;
      // open since the fifth audit). Uploads fit under it by design:
      // photos are re-encoded to a small WebP in the browser, avatars are
      // capped at 2MB, KYC documents at 4MB each with the apply form
      // refusing a submission whose documents total more than
      // MAX_SUBMISSION_BYTES. Keep this in step with that constant.
      bodySizeLimit: '4mb',
    },
  },
