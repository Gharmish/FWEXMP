 ...
 * clears the seeded tables in FK-safe order, then re-inserts, all in ONE
 * transaction so a mid-way failure (e.g. the bookings FK RESTRICT on
 * experiences) leaves the catalog exactly as it was.
 *
 * DESTRUCTIVE GUARD (2026-09 engineering audit DATA-02): this script
 * deletes moments, experiences and hosts. The shared checkout's
 * DATABASE_URL points at the LIVE database, so it refuses to run against
...
function assertSeedAllowed(): void {
  ...
  if (!isLocalDatabase(url) && process.env.SEED_ALLOW_DESTRUCTIVE !== '1') {
    console.error(
      'db:seed refuses to run: DATABASE_URL is not a local database and this script DELETES ' +
        'moments, experiences and hosts before re-inserting. If you really mean the remote ' +
        'database, re-run with SEED_ALLOW_DESTRUCTIVE=1.',
    );
    process.exit(1);
  }
}
