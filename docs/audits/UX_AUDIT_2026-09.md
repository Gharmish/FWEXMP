...
>
> **Remediation status #2 (2026-09-05, later the same day) — the P2/P3 backlog is done** except the owner-decision and out-of-bounds items listed at the end. Five parallel fix packages (global chrome + design system, home/catalog/detail, copy EN+AR, host + account, admin) plus two app-wide sweeps by the lead; `pnpm typecheck`, `pnpm lint` and all 1,530 tests pass; public and host pages re-verified in the browser at 375/1280 px, EN + AR.
>
> - **Fixed P2s:** P2-1 (banner body two lines, privacy link inline, one button row), P2-2 (`--color-pending` → gold-900, eyebrows → gold-800, undefined `-700` gone), P2-3 (hero words ordered widest-first and the cycle settles on the widest, so neither the first paint nor the resting state has a gap; the shorter words still sit in the fixed slot for their 2.4 s each mid-cycle — inherent to the no-reflow design), P2-4 (placeholder tints at the -100/-200 steps + larger glyph), P2-5 (category strip under the search field below `lg`), P2-6 (one `EmptyState` with a `bordered` variant; catalog copy deleted), P2-7–P2-13 (copy, both locales), P2-14 (Arabic plurals), P2-15 (`bidiIsolate` in the host outcome banner), P2-16/P2-17/M14 (`FieldError` wiring, `ConfirmSubmit` on approve/reject/request-changes and the host review reply, distinct reject label), P2-18 (admin editor unsaved-changes guard), P2-19 (rail badges for applications, moderation, disputes, support), P2-21 (`divide-hairline` utility mirroring Tailwind's `divide-y` at 0.5 px, swept across 32 lists in 17 files), P2-22 (259 off-grid spacing utilities in 95 files mapped onto the 8-pt scale: 5→6, 7→8, 9→8, 10→12, 11→12, 14→16), P2-23 (values echo on payout IBAN, dispute, review reply and promo forms), P2-24 (people-directory truncation notice), P2-25 (client-side validation + "Uploading documents" phase on the host application).
> - **Fixed P3s:** P3-1, P3-3, P3-4, P3-5, P3-6, P3-7, P3-9, P3-10, P3-11, P3-12, P3-13, P3-14, P3-15, P3-16, P3-17 (greeting), P3-18, P3-19, P3-20, P3-21, P3-22, P3-23, P3-25, P3-26, P3-29 (Textarea + Select primitives created), P3-31, P3-32, P3-33 (category disable), P3-34; and from the July table M11, M13, M16, M20, M21, M23, M25 (notice), M28, M32, L2, L3, L4 (host bookings + promo codes), L5, L10, L11, L13, L16, L17, L19, L20.
> - **Deferred — owner decisions:** P2-20 (TOTP backup codes / factor reset — security design), P3-24 (WhatsApp emoji rule conflict), P3-27 (Hijri toggle: build or drop from the brief), the Arabic font preload trade-off, P3-2 (homepage length), P3-35 (audience-specific status words are deliberate).
> - **Deferred — outside this change set:** P3-8 and the July H3 (admin bookings SQL counts/pagination) live in files another session has open uncommitted (`phone-input.tsx`, `admin/bookings/*`); the payouts-due rail badge needs `payout-sql.ts` (same reason); M30 (a `md:` two-column detail layout) was tried and reverted — the mobile booking bar overlaps the panel at 768 px, so it needs a design pass; M2/M5/M7/M10 (Card/icon-button/status-map/eyebrow-token migrations) and P3-30 are hygiene sweeps with no user-visible effect, left for a quiet moment; P3-28 was checked and the Originals chip fits at 375 px.
