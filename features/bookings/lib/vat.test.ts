...
describe('vatPortionSar', () => {
  it('extracts the inclusive VAT portion at 15%', () => {
    ...
    expect(vatPortionSar(115)).toBe(15);
    expect(vatPortionSar(230)).toBe(30);
    // 960 inclusive → 960 × 15/115 = 125.22 → 125
    expect(vatPortionSar(960)).toBe(125);
  });

  it('never exceeds the total and handles edge inputs', () => {
    expect(vatPortionSar(0)).toBe(0);
    expect(vatPortionSar(-5)).toBe(0);
    expect(vatPortionSar(1)).toBe(0); // 0.13 rounds to 0
