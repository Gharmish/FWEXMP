        ...
        {items.map((item, i) => (
          <li key={`${item.href}-${i}`}>
            <StaggerItem>
              <Link
                href={item.href}
                className="border-sarat-black/12 rounded-card hover:border-sarat-black/20 group flex items-center justify-between gap-4 border px-6 py-4 transition-colors duration-200"
              >
