    ...
    <nav aria-label={t('nav.menuLabel')} className="flex flex-col gap-6">
      {GROUPS.map((group) => (
        <div key={group.headingKey} className="flex flex-col gap-1">
          <p className="text-sarat-black-600 text-eyebrow px-3 pb-1">
              ...
              return (
                <li key={item.href}>
                  ...
                  >
                    ...
                    {badgeCount > 0 && (
                      <span
                        className="bg-saffron-gold/20 text-sarat-black inline-flex min-w-6 shrink-0 items-center justify-center rounded-full px-2 py-0.5 text-xs font-medium tabular-nums"
