...
import { listReviewsForHost } from '@/features/reviews/queries';
import { HostReplyForm } from '@/app/[locale]/host/reviews/host-reply-form';
...
export default async function HostReviewsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [t, rows] = await Promise.all([getTranslations('hostReviews'), listReviewsForHost()]);
